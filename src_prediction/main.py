...
def prepare_model():
    ...
    model = T5MultiSPModel.load_from_checkpoint('./models/after_finetuning.ckpt')
...
def calculate_output(model, input_string):
    ...
    generated_ids = model.model.generate(inputs.cuda(), num_beams=5, repetition_penalty=1.0, max_length=56, early_stopping=True)
    hyps = [model.tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=False) for g in generated_ids]
